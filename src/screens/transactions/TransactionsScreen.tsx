...
export default function TransactionsScreen() {
  ...
  // Resolve category name for a transaction (id-first, name fallback)
  const resolveCategoryName = (t: (typeof transactions)[0]) => {
    if (t.categoryId) return getCategoryById(t.categoryId)?.name ?? t.category ?? '';
    const cats = t.type === 'expense' ? expenseCategories : incomeCategories;
    return cats.find(c => c.name === t.category)?.name ?? t.category ?? '';
  };
  ...
  const searched = search.trim()
    ? rangeFiltered.filter(
        t =>
          ...
          resolveCategoryName(t).toLowerCase().includes(search.toLowerCase()) ||
    ...
    if (groupBy === 'type')
      return t.type === 'income'
        ...
        : t.type === 'expense'
          ? '🔴 Expense'
          : '🔵 Transfer';
    if (groupBy === 'category') return resolveCategoryName(t) || 'Uncategorized';
